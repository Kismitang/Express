  ...
  methods:{
    addAdmin(){
      if(this.admin.role === 0){
        this.$message.warning("普通员工不允许执行该(新增)操作")
        return;
      }
      this.addDV = true
    },
    ...
    mod(){
      this.$refs.ruleFormRef.validate((valid) => {
        if (valid) {
          ...
          console.log("表单验证失败");
          this.$message.warning("输入的数据不符合要求")
          return false;
        }
      });
    },
    ...
    deleteA(row){
      if(this.admin.role === 0){
        this.$message.error("普通管理员不允许删除员工信息")
        return;
      }
      this.form = row;
      this.deleteDV = true
  ...
  <div>
    ...
    <div>
      ...
      <el-dropdown
        ...
        {{ role ? (role === '1' ? '超级管理员' : '普通管理员') : '全部角色' }}
        <template #dropdown>
          <el-dropdown-menu style= "min-width: 115px;">  <!-- 同步设置下拉菜单宽度 -->
            <el-dropdown-item style="width: 100%" command="">全部角色</el-dropdown-item>
...
<!--            &lt;!&ndash;              <el-input v-model="form.workStatus" style="width: 230px"/>&ndash;&gt;-->
